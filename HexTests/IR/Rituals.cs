...
namespace HexTests.IR
{
	public class Rituals : LowerTestUtilities
	{
		...
		public void Decl_and_Call_Add_a_b()
		{
			...
			Assert.That(list[6].opCode, Is.EqualTo(OpCode.LoadU64Const));
			Assert.That(list[7].opCode, Is.EqualTo(OpCode.LoadU64Const));
			Assert.That(list[8].opCode, Is.EqualTo(OpCode.Param));
			Assert.That(list[9].opCode, Is.EqualTo(OpCode.Param));
			Assert.That(list[10].opCode, Is.EqualTo(OpCode.Call));
			Assert.That(list[11].opCode, Is.EqualTo(OpCode.Copy));
			Assert.That(list[12].opCode, Is.EqualTo(OpCode.Whisper));
			Assert.That(list[13].opCode, Is.EqualTo(OpCode.LoadU64Const));
			Assert.That(list[14].opCode, Is.EqualTo(OpCode.Return));
		}
